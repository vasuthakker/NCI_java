...
public class LoginController {
	...
	@RequestMapping(value = "/loginStatus", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody String getLoginStatus(@RequestBody String jsonInputString, HttpServletRequest request,
			HttpServletResponse response) {
		logger.info("Start of LoginController");
		JsonNode outputNode ;
		try {
			JsonNode inputNode = mapper.readTree(jsonInputString);

			outputNode = loginService.loginDetails(inputNode);
